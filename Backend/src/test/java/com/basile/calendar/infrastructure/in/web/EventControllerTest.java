...
class EventControllerTest {
    ...
    void should_return_events_for_given_date_when_listing() throws Exception {
        ...
        when(listEventsForDay.list(date)).thenReturn(List.of(event));

        mockMvc.perform(get("/api/events").param("date", "2026-09-02"))
    ...
    @Test
    void should_create_event_when_posting_valid_request() throws Exception {
        LocalDate date = LocalDate.of(2026, 9, 2);
        Event created = Event.draft("Point équipe", date, LocalTime.of(15, 0), 90).withId(1L);
